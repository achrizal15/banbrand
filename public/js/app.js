...
const stepWizzard = function () {
    ...
    $("#smartwizard").on("leaveStep", function (e, anchorObject, currentStepIndex, nextStepIndex, stepDirection) {
        ...
        if (stepDirection == "forward" && form.length != 0) {
            if ($("#form-register-seller").parsley().validate({
    ...
    if (form_register_seller.length > 0) {
        ...
        $.ajax({
            ...
            success: function (result) {
                result.forEach(kota => {
                    var newOption = new Option(kota.name, kota.name + "|" + kota.id, false, false);
                    form_register_seller.find("select[name='kota'").append(newOption).trigger('change');
                });
            }
        });
        $(document).on("change", "#select-kota", function () {
            ...
            $.ajax({
                type: "get",
                url: "http://www.emsifa.com/api-wilayah-indonesia/api/districts/" + id_kecamatan + ".json",
                dataType: "json",
                success: function (response) {
                    form_register_seller.find("select[name='kecamatan'").html(new Option("Pilih Satu", "", true, true))
                    form_register_seller.find("select[name='kelurahan'").html(new Option("Pilih Satu", "", true, true))
                    response.forEach(kecamatan => {
                        let newOption = new Option(kecamatan.name, kecamatan.name + "|" + kecamatan.id, false, false);
                        form_register_seller.find("select[name='kecamatan'").append(newOption).trigger('change');
                    });
                }
            });
        })
        $(document).on("change", "#select-kecamatan", function () {
            ...
            $.ajax({
                type: "get",
                url: "http://www.emsifa.com/api-wilayah-indonesia/api/villages/" + id_kelurahan + ".json",
                dataType: "json",
                success: function (response) {
                    form_register_seller.find("select[name='kelurahan'").html(new Option("Pilih Satu", "", true, true))
                    response.forEach(kelurahan => {
                        let newOption = new Option(kelurahan.name, kelurahan.name + "|" + kelurahan.id, false, false);
                        form_register_seller.find("select[name='kelurahan'").append(newOption).trigger('change');
                    });
                }
            });
        })
